...
roomService.getFreeRooms().then(function(payload){$scope.rooms=payload})}var APIconfig={rawUrl:"https://apps.fhict.nl/api/v1",callback:"&callback=JSON_CALLBACK",loginUrl:"https://apps.fhict.nl/link/pluff",url:function(url){// Enclose the given _relative_ url with the absolute url + callback.
...
angular.module("pluffApp.services",[]).factory("lessonService",function(){var data={};return data.generateColor=function(name){// Number between 0 and 1
var generatedNumber=(Math.sin(name.charCodeAt(0))+Math.sin(name.charCodeAt(1))+Math.sin(name.charCodeAt(2)))/6+.5,type=Math.floor(6*generatedNumber),value=Math.floor(81*(Math.sin(2e3*generatedNumber)+1)+64).toString(16);// Choose part of the color based on the outcome of type (so we can control the colors a little)
switch(1===value.length&&(value="0"+value),type){case 0:return"#e440"+value;case 1:return"#"+value+"40e4";case 2:return"#40"+value+"e4";case 3:return"#40e4"+value;case 4:return"#"+value+"e440";case 5:case 6:return"#e4"+value+"40";default:return"#fff"}},data.getTimeTable=function(payload){// Create 52 empty weeks, create within every week 5 days (Mo - Fr) and 14 hours

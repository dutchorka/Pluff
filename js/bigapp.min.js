...
window.location.reload()}}function TimeTableCtrl($scope,$http,lessonService,$window,$location,dataService,timetableData,autocompleteData,ngDialog){// Get the personal schedule from the API
$scope.weeks=lessonService.getTimeTable(timetableData.data),$scope.tableTitle=timetableData.title,// List of the breaks and the duration. The first break is after the second hour and is 20 minutes.
$scope.hourBreaks=[0,0,20,0,0,0,0,10,0,0,15,0,20,0,0],// Fontys starts at 8.45
$scope.dayStartTime=moment().hour(8).minute(45).second(0),// And ends at 21.40
$scope.dayEndTime=moment().hour(21).minute(40).second(0),// Set the default used weeknumber (without leading zero). In the weekend, use the next week number
$scope.weekNumberUsed=parseInt(0===moment().day()||6===moment().day()?moment().add(1,"w").format("w"):moment().format("w")),// Set the default used year number
...
$scope.isOldWeek=function(){return $scope.weekNumberUsed<=$scope.weekNumber().current&&$scope.yearUsed===$scope.weekNumber().yearCurrent?!0:!1},// Add the resulting array in the global scope for the autocomplete plugin to use it
$scope.searchAuto=autocompleteData,// Fired when a search suggestion is selected
...
dataService.getTeacher(teacherAbr).then(function(payload){var data=payload.data;ngDialog.open({template:"partials/dialog-teacher.html",data:data})})},$scope.calculateLine=function(){var now=moment(),percentageComplete=(now-$scope.dayStartTime)/($scope.dayEndTime-$scope.dayStartTime)*100,percentageRounded=Math.round(100*percentageComplete)/100;return percentageRounded+"%"},window.setInterval($scope.calculateLine(),6e4)}// Holidays dialog
...
function RoomsCtrl($scope,roomService){// Load all the rooms with occupied information if it isn't sunday (API gives an error on sundays)
var isSunday=0===moment().day();isSunday||roomService.getFreeRooms().then(function(payload){$scope.rooms=payload})}var APIconfig={rawUrl:"https://apps.fhict.nl/api/v1",callback:"callback=JSON_CALLBACK",loginUrl:"https://apps.fhict.nl/link/pluff"+("webduck.nl"===window.location.host?"-dev":""),url:function(url){// Enclose the given _relative_ url with the absolute url + callback.
return this.rawUrl+url+(-1===url.indexOf("?")?"?":"&")+this.callback}},encode=function(url){return encodeURIComponent(url).replace(/\//g,"%2F")};angular.module("pluffApp",["pluffApp.controllers","pluffApp.services","ngRoute","ngCookies","pascalprecht.translate","angucomplete-alt","ngAnimate","ngDialog"]).config(function($routeProvider,$locationProvider){$routeProvider.when("/",{templateUrl:"partials/timetable.html",controller:"TimeTableCtrl",resolve:{// Load the autocomplete data first.
// If this data can't be loaded the user isn't authenticated yet
autocompleteData:function(dataService){return dataService.getSuggestions().then(function(payload){return payload.data})},// Load the timetable JSON before the controller
timetableData:function(dataService){return dataService.getTimeTable("/me").then(function(payload){return payload.data})}}}).when("/search/:category/:query",{templateUrl:"partials/timetable.html",controller:"TimeTableCtrl",resolve:{autocompleteData:function(dataService){return dataService.getSuggestions().then(function(payload){return payload.data})},timetableData:function($route,dataService){var categoryUrl,queryUrl=$route.current.params.query;// Load correct API url
switch($route.current.params.category){case"room":categoryUrl="room";break;case"class":categoryUrl="class";break;case"teacher":categoryUrl="teacher/abbreviation";break;case"subject":categoryUrl="subject";break;case"class":categoryUrl="class";break;default:categoryUrl="query"}return dataService.getTimeTable("/"+categoryUrl+"/"+queryUrl).then(function(payload){return payload.data})}}}),// We don't want no fake hashbangs we want the real shite
$locationProvider.html5Mode(!0)}).config(function($translateProvider){// Lazyload the language files
...
$translateProvider.useCookieStorage()}).run(function($rootScope){$rootScope.encode=function(url){return encodeURIComponent(url).replace(/\//g,"%2F")}}),/* Services */
angular.module("pluffApp.services",[]).factory("lessonService",function(){var data={};return data.generateColor=function(name){// Loop every character and multiply with the generator seed
for(var total=6e6,i=0;i<name.length;i++)total*=name.charCodeAt(i);var color=total.toString(16).substr(2,6);return"#"+color},data.getTimeTable=function(payload){// Create 52 empty weeks, create within every week 5 days (Mo - Fr) and 14 hours
...
var lessonData={start:start.format("H:mm"),end:end.format("H:mm"),date:start.format("YYYY-MM-DD"),teacher:lesson.teacherAbbreviation.toLowerCase(),subject:lesson.subject.toLowerCase(),room:lesson.room,classes:lesson.classes,color:data.generateColor(lesson.subject)};// Select the current hour and push the new lesson to it
// Array keys are zero based, and skipping the first key results in a hell
weeks[startWeeknumber-1][startDaynumber-1][hourNumber-1].lessons.push(lessonData)}}}),weeks},data}).factory("dataService",function($http){return{getSuggestions:function(){return $http.jsonp(APIconfig.url("/schedule/autocomplete")).error(function(){// User isn't logged in, so redirect to portal
...
window.location=APIconfig.loginUrl})},getTimeTable:function(input){return $http.jsonp(APIconfig.url("/schedule"+input+"?expandTeacher=false&IncludeStartOfWeek=true&daysAhead=90"))},getTeacher:function(teacher){return $http.jsonp(APIconfig.url("/people/abbreviation/"+teacher))}}}).factory("holidayService",function($http,$log,$q){return{getHolidays:function(){// Get the json with all the holiday dates in it

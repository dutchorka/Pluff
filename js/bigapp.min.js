...
return this.rawUrl+url+(-1===url.indexOf("?")?"?":"&")+this.callback}},encode=function(url){return encodeURIComponent(url).replace(/\//g,"%2F")};angular.module("pluffApp",["pluffApp.controllers","pluffApp.services","ngRoute","ngCookies","pascalprecht.translate","angucomplete-alt","ngAnimate","ngDialog"]).factory("httpRequestInterceptor",function($q){return{responseError:function(rejection){// This is temporary. TODO: Redirect only if the user isn't logged in.
...
weeks[startWeeknumber-1][startDaynumber-1][hourNumber-1].lessons.push(lessonData)}}}),weeks},data}).factory("dataService",function($http){return{getTimeTable:function(input){return $http.jsonp(APIconfig.url("/schedule"+input+"?expandTeacher=false&IncludeStartOfWeek=true&daysAhead=90"))},getTeacher:function(teacher){return $http.jsonp(APIconfig.url("/people/abbreviation/"+teacher))},getSuggestions:function(){return $http.jsonp(APIconfig.url("/schedule/autocomplete"))}}}).factory("holidayService",function($http,$log,$q){return{getHolidays:function(){// Get the json with all the holiday dates in it
